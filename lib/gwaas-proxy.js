...
function GwaasProxy(done) {

    var proxy = this;
    proxy.checkpoint = new ConfigCheckpoint(schema.config.gateway.key,defaults,['imei']).start(function(event,gateway){
        if (event !== 'ready') return;

        var redis = proxy.checkpoint.redis;
