...
SystemChecker.prototype.findSerialPorts = function(callback){
    ...
    fs.readdir(PORT_DIRECTORY,function(error,files){
        if (self.verbose && error) logger.info('find error:' + error);
...
SystemChecker.prototype.chooseBestPorts = function(){
    ...
    function findBest(key,callback){
        ...
        if (!choice) {
            ...
            peripheral
                .on('retry',function(reason){ if (self.verbose) logger.info('retry:' + reason); done(); })
                .on('error',function(error){ if (self.verbose) logger.info('error:' + error); done(); })
...
SystemChecker.prototype.collectModemInfo = function(){
    ...
    peripheral
        ...
        .on('error',function(error){
            logger.info('modem error: ' + error);
            process.exit(1);
        })
        .on('retry',function(error){
            logger.info('modem retry: ' + error);
            process.exit(1);
        })
        .on('ready',function(){
            var index = -1;
    ...
    function sendExpect(label,command,prefix,done){
        ...
        peripheral.writeBuffer(command + COMMAND_SUFFIX,function(error){
            if (error) logger.info('callback error: ' + error);
        });
    }
