...
DataReader.prototype._onStart = function() {
    var self = this;
    var lastData = null;
    self.device.on('retry',self.retryListener = function(reason) {
        logger.error('start error: ' + reason);
        self.emit('note','retry');
        self.emit('retry',reason);
    });
    self.device.on('ready',self.readyListener = function(){
        self.emit('note','ready');
        self.emit('ready',self);
    });
    self.device.on('error',self.errorListener = function(error) {
        logger.error('read error: ' + error);
        self.emit('note','error');
        self.emit('error',error);
    });
    self.device.on('data',self.dataListener = function(data){
        var event = 'unknown';
...
DataReader.prototype._onStop = function() {
    this.device.removeListener('retry',this.retryListener);
    this.device.removeListener('ready',this.readyListener);
    this.device.removeListener('error',this.errorListener);
    this.device.removeListener('data',this.dataListener);
    this.device.close();
};
