...
ConfigCheckpoint.prototype.attemptCheck = function(callback){
    ...
    self.redis.hgetall(schema.config.key,function(err,hash){
        if (!err && hash && _.difference(self.required,_.intersection(self.required,Object.keys(hash))).length == 0)
            callback('ready',self.buildConfig(hash));
        else {
...
ConfigCheckpoint.prototype.buildConfig = function(hash) {
    var result = {};
    for (var field in this.hashkeys) {
        var keyspec = this.hashkeys[field];
        var value = hash[keyspec.key] || keyspec.default;
        if (keyspec.type == 'number')
            value = +value;
        result[field] = value;
    }
    return result;
