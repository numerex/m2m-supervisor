...
var gwaas = require('./gwaas-settings');

function GwaasTransmitter(gateway,config) {
    var self = this;
...
GwaasTransmitter.prototype.started = function(){
    return !!this.startCalled;
};

GwaasTransmitter.prototype.ready = function(){
    return !!this.gateway;
};

GwaasTransmitter.prototype.start = function(note) {
...
GwaasTransmitter.prototype.stop = function() {
...
GwaasTransmitter.prototype.checkQueue = function(){
...
GwaasTransmitter.prototype.createMessage = function(attributes,lastSequenceNumber) {
    var eventCode = attributes.eventCode || gwaas.EventCodes.heartbeat;
...
GwaasTransmitter.prototype.transmitMessage = function(message){
...
GwaasTransmitter.prototype.asyncCheckQueue = function(){
    setTimeout(this.checkQueueCallback,1);
};

GwaasTransmitter.prototype.redisCheckResult = function(err,value,callback) {
    if (!err)
        callback(value);
    else {
...
GwaasTransmitter.prototype.redisLogError = function(err,value,callback) {
    if (err) {
        logger.error('redis error: ' + err);
        this.stats.increment('error');
        this.noteEvent('error');
...
GwaasTransmitter.prototype.safeParseJSON = function(contents) {
    try {
...
module.exports = GwaasTransmitter;
