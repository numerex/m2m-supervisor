...
function TelnetDevice(config) {
    ...
    self.dataEventCallback = function(data){ self.noteEvent('data',data); }
}
...
TelnetDevice.prototype.attemptOpen = function(){
    ...
    try {
        self.timeout = null;
        self.client = net.connect({port: self.port,host: self.port},function(){
...
TelnetDevice.prototype.writeBuffer = function(buffer,callback){
    ...
    try {
        if (this.client)
            this.client.write(buffer,function(){ callback(null); });
        else
            callback('not ready');
    } catch (e) {
        callback(e);
